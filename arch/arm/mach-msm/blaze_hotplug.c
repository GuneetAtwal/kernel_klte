 ...
 *
 * Simple Hotplug driver based on showp1984's msm_mpdecision
 */
...
#define BLAZE_HOTPLUG "blaze_hotplug"
...
static void blaze_hotplug_suspend(struct work_struct *work)
{
	int cpu;
...
static void __ref blaze_hotplug_resume(struct work_struct *work)
{
	int cpu;
...
static struct attribute *blaze_hotplug_control_attributes[] =
{
...
static struct attribute_group blaze_hotplug_control_group =
{
	.attrs  = blaze_hotplug_control_attributes,
};

static struct miscdevice blaze_hotplug_control_device =
{
	.minor = MISC_DYNAMIC_MINOR,
	.name = "blaze_hotplug_control",
};
...
static int __devinit blaze_hotplug_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct hotplug_tunables *t = &tunables;

	wq = alloc_workqueue("blaze_hotplug_workqueue", WQ_FREEZABLE, 1);
	...
	ret = misc_register(&blaze_hotplug_control_device);
	...
	ret = sysfs_create_group(&blaze_hotplug_control_device.this_device->kobj,
			&blaze_hotplug_control_group);
	...
	INIT_WORK(&resume, blaze_hotplug_resume);
	INIT_WORK(&suspend, blaze_hotplug_suspend);
...
static struct platform_device blaze_hotplug_device = {
	.name = BLAZE_HOTPLUG,
	.id = -1,
};

static int blaze_hotplug_remove(struct platform_device *pdev)
{
...
static struct platform_driver blaze_hotplug_driver = {
	.probe = blaze_hotplug_probe,
	.remove = blaze_hotplug_remove,
	.driver = {
		.name = BLAZE_HOTPLUG,
		.owner = THIS_MODULE,
	},
...
static int __init blaze_hotplug_init(void)
{
	int ret;

	ret = platform_driver_register(&blaze_hotplug_driver);
	...
	ret = platform_device_register(&blaze_hotplug_device);
...
static void __exit blaze_hotplug_exit(void)
{
	platform_device_unregister(&blaze_hotplug_device);
	platform_driver_unregister(&blaze_hotplug_driver);
}

late_initcall(blaze_hotplug_init);
module_exit(blaze_hotplug_exit);
